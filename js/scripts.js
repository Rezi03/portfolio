document.addEventListener("DOMContentLoaded", function() {
  ...
  let lastSY = window.scrollY, skew = 0;
  (function decaySkew() {
    skew *= 0.8;
    document.querySelectorAll('section').forEach(s => {
      s.style.transform = Math.abs(skew) > 0.05 ? `skewY(${skew}deg)` : '';
    });
    requestAnimationFrame(decaySkew);
  })();
  window.addEventListener('scroll', () => {
    const delta = window.scrollY - lastSY;
    lastSY = window.scrollY;
    skew = Math.max(-2, Math.min(2, delta * 0.045));
  }, { passive: true });
